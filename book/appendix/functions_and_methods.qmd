...
| `max(iterable, *args)` | ترجع أكبر عنصر في الكائن القابل للتكرار أو أكبر بين وسيطين أو أكثر. |
| `min(iterable, *args)` | ترجع أصغر عنصر في الكائن القابل للتكرار أو أصغر بين وسيطين أو أكثر. |
...
| `float.is_integer()` | ترجع `True` إذا كانت قيمة العدد العشري عددًا صحيحًا، و `False` خلاف ذلك. |
...
| الفعل | الوصف |
| --- | --- |
| `list.append(x)` | تضيف العنصر `x` إلى نهاية القائمة. |
| `list.extend(iterable)` | تضيف جميع العناصر من الكائن القابل للتكرار إلى نهاية القائمة. |
| `list.insert(i, x)` | تُدخل العنصر `x` في الموضع المحدد `i`. |
| `list.remove(x)` | تزيل أول عنصر في القائمة تكون قيمته مساوية لـ `x`. |
| `list.pop([i])` | تزيل العنصر الموجود في الموضع المحدد `i` وتعيده. إذا لم يتم تحديد `i`، فإنها تزيل وتعيد آخر عنصر في القائمة. |
| `list.clear()` | تزيل جميع العناصر من القائمة. |
| `list.index(x[, start[, end]])` | ترجع فهرس أول عنصر تكون قيمته مساوية لـ `x`. |
| `list.count(x)` | ترجع عدد مرات ظهور `x` في القائمة. |
| `list.sort(*, key=None, reverse=False)` | ترتب القائمة في مكانها (تعدل القائمة الأصلية). |
| `list.reverse()` | تعكس ترتيب عناصر القائمة في مكانها. |
| `list.copy()` | ترجع نسخة سطحية من القائمة. |
