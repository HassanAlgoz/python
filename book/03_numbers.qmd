...
You can reassign a variable using the result of an operation. For example:
...
Side note: see [Augmented assignment statements](https://docs.python.org/3/reference/simple_stmts.html#augmented-assignment-statements) for details in which they actually differ.

Try the code below to see the result:
...
Note that `i++` is not valid in Python. The following will raise an error:
