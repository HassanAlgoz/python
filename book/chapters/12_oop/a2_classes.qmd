...
3. **التخصيص**: حيث نبني صنف **الطابور** باختيار إجراءات محددة من السلسلة على نسق ما
...
### دخول النوع في صنف باعتبار إجراءاته

ولأن هذا المركَّب مكوَّن من أفراد، فنريده أن يُشابه **الأنواع الحاوية**، التي لها إجراءات معيَّنة. وذلك ممكنٌ باستعمال الإجراءات المخصصة:
...
أما قبول الإجراء `len` لأنواع مغايرة عن بعضها فتسمَّى خاصيَّة **تعدد الأشكال** [(Polymorphism)](https://en.wikipedia.org/wiki/Polymorphism_(computer_science)): أي أن الإجراء الواحد يقبل أنواع متعددة في الحقيقة لكنَّها تُظهر نفس السلوك الخارجي. بعبارة أخرى نقول:
...
تبين بهذا محاكاة بعض إجراءات الحاويات الأساسية مثل `list` و`set` و`tuple` و`str` و`dict`. وقد تقدَّم معنا أن النقطة تستطيع محاكاة الأنواع العددية باستعمال الإجراء المخصص للجمع `+` الذي هو `__add__`.

## تخصيص النوع العام

يكثر استعمال **الطابور** كهيكل بيانات لحل مجموعة من المسائل بطريقة سهلة. نستعرض في هذا المثال كيفية تخصيص القائمة (`list`) ذات الإجراءات الكثيرة من الحذف والإضافة والتعديل، لنوع له إجراءات مختلفة لكنها مبنية عليها.
...
- ثم الإجراء `enqueue` يتم بواسطة فعل القائمة `append`
- والإجراء `dequeue` يتم بواسطة فعل القائمة `pop`
