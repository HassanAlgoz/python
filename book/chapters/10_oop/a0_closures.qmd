...
ويُمكن استعماله هكذا أيضًا في خطوة واحدة، هكذا:

```{python}
print("Squares:", make_power(2)(2), make_power(2)(3))
print("Cubes:", make_power(3)(2), make_power(3)(3))
```

وكذلك يمكن إسناد متغيرات يحتفظ بها الفعل الذي يتم إنشاؤه.
ونمثل على ذلك بإنشاء العداد `make_counter` على النحو التالي:
...
**المزيِّن** (decorator) هو الفعل الذي يأخذ فعلاً ويرجع لنا فعلاً جديدًا منه.
وتعريفه العام نحو:
...
2. عرفنا فعلاً جديدًا يسمَّى `memoized` يحسب نتيجة الفعل المُعطى `f` بالمدخل `x`، ويحفظها في `cache` إذا لم يكن موجودة.
