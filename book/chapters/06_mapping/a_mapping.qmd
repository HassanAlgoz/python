...
أفعال التحقق تقبل متوالية (Iterable)، وتنتج قيمة صواب (`True`) أو خطأ (`False`).
وقد عرفنا أن الصف والقائمة والقاموس كلها من جنس المتوالية.
وهما فعلان:
...
وذلك باعتبار ما تصير إليه المتوالية.
...
if any(a in answer for a in answer_set):
	print("صواب")
else:
	print("خطأ")
```
