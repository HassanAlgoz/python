...
أما الرؤوس الأخر فقد أضافها العميل من نفسه:
...
- `x-api-key: 1234567890` هو مفتاح التطبيق الذي يربط هذا الطلب بالحساب المسجَّل في الخدمة

وبعد ذلك نرسل الطلب بالفعل `send` ونحصل على جواب `response`:
...
ولكن هذا النص لا يمكن التعامل معه كما هو، لذلك نستخدم الفعل `json()` لتفسيره إلى شيء في بايثون (قائمة):
...
والرابط يشبه استدعاء الفعل. لاحظ أننا في بايثون نستدعي الفعل `pow` من الوحدة `math` ونمرر العوامل `2, 3` إليه على هذا النحو:
