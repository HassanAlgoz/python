...
xs = [20, 10, 30, 40]
ys = [20, 10, 30, 40]
assert list.sort(xs) == ys.sort()
assert list.append(xs, 50) == ys.append(50)
```
...
فينتج لنا بطلب `calculate_grade(95)` نسخة معيَّنة من تفاصيل الفعل، نسمّيها [**ظرف التنفيذ**](https://docs.python.org/3/reference/executionmodel.html#structure-of-a-program) التنفيذ؛ يكون فيه المتغير `score=95` كأنه هكذا:
...
تأمل المثال التالي الذي يأخذ عاومل ثم لا يرجع بشيء، لكنه يُظهر النتيجة على الشاشة، وقد كتبنا فيه جملة الرجوع `return None` لكنها مقدَّرة على أية حال.
